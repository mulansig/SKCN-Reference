		...
		const unsigned char * sk1)
{
	...
	for(i=0; i<DIM_s; i++, p += BYTES_s)
		unpack_polyeta(s+i, p);
	
	for(i=0; i<DIM_e; i++, p += BYTES_e)
		unpack_polyeta(e+i, p);
}
...
void pack_c(unsigned char * p, const Polynomial *c)
{
	uint64_t signs, mask;
	uint32_t i, j; 
...
void unpack_c(Polynomial * c, const unsigned char * p)
{
	uint64_t signs, mask;
	uint32_t i, j; 
		...
		const unsigned char * signature)
{
	...
	unpack_z(z, p);
	p += SIZE_z; unpack_h(h, p);
	p += SIZE_h; unpack_c(c, p);
}
